...
function K8s(K8s) {
    ...
    K8s.executeCommand = async function (command, JSONOutput) {
        ...
        var kubectl_command = configs.then(function (kube_configs) {
            ...
            return "kubectl --token " + kube_configs["kubernetes_token"] + " --kubeconfig=/dev/null --server=\"" + kube_configs["kubernetes_api_protocol"] + "://" + kube_configs["kubernetes_api_address"] + ":" + kube_configs["kubernetes_api_port"] + "\" --insecure-skip-tls-verify=true " + command + formatting;
        });
                ...
                exec(command, {maxBuffer: 1024 * 10000}, function (error, stdout, stderr) {
                    try {
                        if(error){
                            ...
                            resolve({'error': JSON.parse(error.toString())});
                        }
                        else{
                        ...
                        resolve(JSON.parse(exception.toString()));
                    }
                });
            });
        });
    ...
    K8s.setDeploymentSize = async function (deployment_name, cpu_cap, memory_cap) {
        var configs = getKubernetesConfiguration(K8s);

        var set_resource_limits = "--limits=";
        var set_resource_requests = "--requests=";
        var resource_counter = 0;


        if (cpu_cap !== undefined) {
            set_resource_limits += "cpu=" + cpu_cap + "m";
            set_resource_requests += "cpu=" + cpu_cap + "m";
            resource_counter++;
        }

        if (memory_cap !== undefined) {
            if (resource_counter == 1) {
                set_resource_limits += ",";
                set_resource_requests += ",";
            }

            set_resource_limits += "memory=" + memory_cap;
            set_resource_requests += "memory=" + memory_cap;
        }

        var kubectl_command = configs.then(function (kube_configs) {
            const kubectl_prepration = "kubectl --token " + kube_configs["kubernetes_token"]
                + " --server=\"" + kube_configs["kubernetes_api_protocol"]
                + "://" + kube_configs["kubernetes_api_address"]
                + ":" + kube_configs["kubernetes_api_port"]
                + "\" --insecure-skip-tls-verify=true ";


            var cmd = kubectl_prepration + "get deployments --all-namespaces"; //get all namespaces
            cmd += " | tail -n +2 "; //remove first line (column names)
            cmd += " | awk '{if ($2==\"" + deployment_name + "\"){ print $1 }}' "; //find deployment_name's namespace
            cmd += " | xargs -I % sh -c '" + kubectl_prepration
                + "set resources deployment.v1.apps/" + deployment_name
                + " --namespace=% " + set_resource_limits + " "
                + set_resource_requests + "'"; // set resources for the deployment in namespace
    ...
    K8s.remoteMethod('setDeploymentSize', {
        accepts: [{"arg": "deployment_name", "type": "string", "required": true},
            {"arg": "cpu_cap", "type": "number", "required": false, "description": "in milicpu"},
            {"arg": "memory_cap", "type": "number", "required": false, "description": "in bytes"}],
        returns: {arg: 'result', type: 'json'},
        http: {path: '/setDeploymentSize', verb: 'post'}
    });
