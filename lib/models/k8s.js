...
function K8s(K8s) {
    ...
    K8s.executeCommand = async function (command) {
        var kube_configs = getKubernetesConfiguration(K8s);

        var kubectl_command = "kubectl --token " + kube_configs["kubernetes_token"] + " --server=\"" + kube_configs["kubernetes_api_protocol"] + "://" + kube_configs["kubernetes_api_address"] + ":" + kube_configs["kubernetes_api_port"] + "\" --insecure-skip-tls-verify=true " + command + " -o json";
